...
void print_context(MD2_CTX *ctx)
{
    int i;
    for (i = 0; i < 16; i++) {
        printf("%2x", ctx->data[i]);
    }
    printf("|");
    for (i = 0; i < 48; i++) {
        printf("%2x", ctx->state[i]);
    }
    printf("|");
    for (i = 0; i < 16; i++) {
        printf("%2x", ctx->checksum[i]);
    }
    printf("\n");
}
...
int md2_test()
{
    ...
    md2_init(&ctx);
    printf("Context after init:\n");
    print_context(&ctx);
    printf("ctx_len %d\n", ctx.len);

    md2_update(&ctx, text1, strlen(text1));
    printf("Context after update:\n");
    print_context(&ctx);
    printf("ctx_len %d\n", ctx.len);

    md2_final(&ctx, buf);
    printf("Context after final:\n");
    print_context(&ctx);
    printf("ctx_len %d\n", ctx.len);
