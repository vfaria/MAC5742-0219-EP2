...
__device__ BYTE revchar(char ch)
{
    if (ch >= 'A' && ch <= 'Z')
        ch -= 'A';
    else if (ch >= 'a' && ch <='z')
...
__global__ void base64_encode(const BYTE in[], BYTE out[], size_t len, int newline_flag, size_t* buf_len)
{
    ...
    if (out == NULL && idx == 3){
        ...
        if (idx < blk_ceiling){
            ...
            if (newline_flag) newline_count = idx / NEWLINE_INVL;

            out[idx2 + newline_count]     = charset[in[idx] >> 2];
            out[idx2 + newline_count + 1] = charset[((in[idx] & 0x03) << 4) | (in[idx + 1] >> 4)];
            out[idx2 + newline_count + 2] = charset[((in[idx + 1] & 0x0f) << 2) | (in[idx + 2] >> 6)];
            out[idx2 + newline_count + 3] = charset[in[idx + 2] & 0x3F];
...
/*
__host__ size_t base64_decode(const BYTE in[], BYTE out[], size_t len)
{
    BYTE ch;
